...
class PaymentRepository extends BaseRepository
{
    ...
    {
        parent::__construct('pagamento', 'idpagamento', true);
    }

    public function create(int $pedidoId, string $gateway, string $metodo, string $idempotencyKey, float $valor): int
    {
        $existing = $this->db->prepare('SELECT idpagamento FROM pagamento WHERE academia_id=? AND idempotency_key=? LIMIT 1');
        $existing->execute([$this->academyId(), $idempotencyKey]);
        if ($row = $existing->fetch()) {
            return (int) $row['idpagamento'];
        }
        $pedido = $this->db->prepare('SELECT valor_total FROM pedido_comercial WHERE idpedido=? AND academia_id=? LIMIT 1');
        ...
        $stmt = $this->db->prepare('INSERT INTO pagamento (academia_id, pedido_id, gateway, idempotency_key, metodo, valor) VALUES (?, ?, ?, ?, ?, ?)');
    ...
    public function applyGatewayResult(string $gateway, string $externalId, string $status): bool
    {
        ...
        $stmt = $this->db->prepare('UPDATE pagamento SET external_id=?, status=? WHERE gateway=? AND academia_id=?');
